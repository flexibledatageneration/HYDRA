...
def main(datasets, models):
    ...
    for dataset in datasets:
        for model_name in models:
            ...
            if not os.path.exists(dataset_recbole_data_folder):
                ...
                #data[0] = data.groupby(0).count() > 10
            ...
            if not os.path.exists("recbole/{}".format(datasets[0])):
                os.makedirs("recbole/{}".format(datasets[0]))

            if not os.path.exists("recbole/{}/tmp".format(datasets[0])):
                os.makedirs("recbole/{}/tmp".format(datasets[0]))
            ...
            tmp_path = "recbole/{}/tmp/results_{}.tsv".format(datasets[0], dataset)
    ...
    for metric in metrics:
        ...
        for topk in topks:
            ...
            new_df = []
            for d in datasets:  # for g in df_metric_topk_grouped:
                g = df_metric_topk_grouped.get_group(d)
                new_df.extend(g.to_numpy())
            # df_metric_topk_grouped.to_frame().to_csv('aaa.csv')
            ...
            new_df[["Dataset", "RS-Value"]].to_csv('recbole/{}/{}_{}.tsv'.format(datasets[0], metric, topk), sep="\t", index=False)
...
if __name__ == '__main__':
    ...
    datasets = ["movielens-1m_synthetic"]
